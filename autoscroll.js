...
function autoScrollTo(element) {
  ...
  autoScrollTimer = window.setTimeout(() => {
    ...
    element.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start', inline: 'nearest' });
  }, 180);
}

function autoScrollNewestVisibleTarget() {
  const visible = AUTO_SCROLL_TARGET_IDS.map((id) => document.getElementById(id)).filter(autoScrollIsVisible);
  if (visible.length) autoScrollTo(visible[visible.length - 1]);
}
...
const autoScrollObserver = new MutationObserver((mutations) => {
  ...
  for (const mutation of mutations) {
    if (mutation.type === 'attributes') {
      ...
      if (target instanceof Element && AUTO_SCROLL_TARGET_IDS.includes(target.id) && autoScrollIsVisible(target)) candidate = target;
    }
  ...
  if (candidate) autoScrollTo(candidate); else autoScrollNewestVisibleTarget();
});

autoScrollEnsureControl();
autoScrollObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['hidden', 'class', 'style'] });
